...
class WorkQueue {
	...
	void validate() {
		synchronized (workLock) {
			// Verify that all running items know they are running, and counts
			// match
			int count = 0;
	...
	private void finishItemAndStartNew(WorkNode finished) {
		...
		synchronized (workLock) {
			...
			if (runningCount < maxConcurrent) {
				...
				if (ready != null) {
					// The Queue reassignments are necessary since 'ready' might
					// have been
					// added / removed from the front of either queue, which
					// changes its
					// respective head.
