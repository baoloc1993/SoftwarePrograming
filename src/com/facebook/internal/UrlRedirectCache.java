...
class UrlRedirectCache {
	...
	synchronized static FileLruCache getCache(Context context)
			throws IOException {
		if (urlRedirectCache == null) {
			urlRedirectCache = new FileLruCache(
					context.getApplicationContext(), TAG,
	...
	static void cacheUriRedirect(Context context, URI fromUri, URI toUri) {
		...
		try {
			FileLruCache cache = getCache(context);
			redirectStream = cache.openPutStream(fromUri.toString(),
					REDIRECT_CONTENT_TAG);
	...
	static void clearCache(Context context) {
		try {
			...
			Logger.log(LoggingBehavior.CACHE, Log.WARN, TAG,
					"clearCache failed " + e.getMessage());
		}
	}
}
