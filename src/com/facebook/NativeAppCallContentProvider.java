 ...
 *
 * <p>
 * Note that this ContentProvider is only necessary if an application wishes to
 * attach images, etc., that are stored in memory and do not have another way to
 * be referenced by a content URI. For images obtained from, e.g., the Camera or
 * Gallery, that already have a content URI associated with them, use of this
 * class is not necessary.
 * </p>
 *
 * <p>
 * If an application wishes to attach images that are stored in-memory within
 ...
 * . See the {@link NativeAppCallContentProvider#getAttachmentUrl(String)
 * getContentProviderName} method.
 * </p>
 */
public class NativeAppCallContentProvider extends ContentProvider {
	private static final String TAG = NativeAppCallContentProvider.class
			.getName();
	...
	interface AttachmentDataSource {
		File openAttachment(UUID callId, String attachmentName)
	...
	public static String getAttachmentUrl(String applicationId, UUID callId,
		...
		return String.format("%s%s/%s/%s", ATTACHMENT_URL_BASE, applicationId,
				callId.toString(), attachmentName);
	}
	...
	public android.os.ParcelFileDescriptor openFile(android.net.Uri uri,
		...
		try {
			File file = dataSource.openAttachment(
	...
	Pair<UUID, String> parseCallIdAndAttachmentName(Uri uri) {
		try {
			// We don't do explicit format checking here. Malformed URIs may
			// generate NullPointerExceptions or
			// array bounds exceptions, which we'll catch and return null. All
			// of these will result in a
